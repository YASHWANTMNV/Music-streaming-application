...
import { getMusicById, deleteMusic, addItemToFavorite } from '../Services/api'; // Import addFavorite API
...
const UserViewMusicById = () => {
    ...
    const handleAddToFavorite = async () => {
        try {
            const response = await addItemToFavorite(userId, music.id);
            if (response) {
                alert('Added to favorites successfully!');
                setIsFavorite(true); // Update favorite status visually
            }
        } catch (error) {
            if (error.response && error.response.status === 409) {
                // If the backend sends a 409 conflict for duplicates
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.controlsContainer}>
                ...
                <IconButton
                    color={isFavorite ? 'secondary' : 'primary'} // Change color based on favorite status
...
export default UserViewMusicById;
